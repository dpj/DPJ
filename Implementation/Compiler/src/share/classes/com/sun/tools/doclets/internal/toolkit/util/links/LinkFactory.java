...
public abstract class LinkFactory {
    ...
    public LinkOutput getTypeParameterLinks(LinkInfo linkInfo, boolean isClassLabel) {
        ...
        Type[] tvars;
        List<RPL> rvars;
        boolean displayTR = true;
        if (linkInfo.executableMemberDoc != null) {
            tvars = linkInfo.executableMemberDoc.typeParameters();
            rvars = linkInfo.executableMemberDoc.regionParameters();
        } else if (linkInfo.type != null && 
                linkInfo.type.asParameterizedType() != null){
            tvars =  linkInfo.type.asParameterizedType().typeArguments();
            rvars = linkInfo.type.asParameterizedType().regionArguments();
            displayTR = false;
        } else if (linkInfo.classDoc != null){
            tvars = linkInfo.classDoc.typeParameters();
            rvars = linkInfo.classDoc.regionParameters();
        } else {
            //Nothing to document.
            return output;
        }
        boolean varsExist = tvars.length > 0 || rvars.nonEmpty();
        if (((linkInfo.includeTypeInClassLinkLabel && isClassLabel) || 
            ...
            if (tvars.length > 0 && displayTR) {
        	linkInfo.displayLength += 5;
        	output.append("type ");
            }
            for (int i = 0; i < tvars.length; i++) {
                ...
                output.append(getTypeParameterLink(linkInfo, tvars[i]));
            }
            if (rvars.nonEmpty()) {
        	if (tvars.length > 0) {
        	    linkInfo.displayLength += 1;
        	...
        	if (displayTR) {
        	    linkInfo.displayLength += 7;
        	    output.append("region ");
        	}
        	do {
        	    String out = rvars.head.toString();
        	    linkInfo.displayLength += out.length();
        	    output.append(out);
        	    rvars = rvars.tail;
        	} while(rvars.nonEmpty());
            }
            linkInfo.displayLength += 1;
