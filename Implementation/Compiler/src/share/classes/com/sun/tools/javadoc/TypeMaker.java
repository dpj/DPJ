...
public class TypeMaker {
	...
	public String dimension() {
	    // DPJ:  We need to print out the region and index var stuff for each
	    // dimension.
	    Type baseType = arrayType;
	    while (baseType instanceof ArrayType) {
		baseType = ((ArrayType) baseType).elemtype;
	    }
	    int start = baseType.toString().replaceAll("<","&lt;").replaceAll(">","&gt;").length();
	    String arrayString = arrayType.toString().replaceAll("<","&lt;").replaceAll(">","&gt;");
	    // Since the "dimension" is everything in the type after the dimension 0 element type,
	    // return a substring starting at the position after the dimension 0 element type.
	    return (start < arrayString.length()) ?
		    arrayString.substring(start) : ""; 
	    /*
	    // Pre-DPJ code:
	    StringBuffer dimension = new StringBuffer();
