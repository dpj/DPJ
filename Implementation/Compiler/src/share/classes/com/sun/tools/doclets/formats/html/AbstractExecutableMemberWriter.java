...
public abstract class AbstractExecutableMemberWriter extends AbstractMemberWriter {
    ...
    protected void writeEffects(ExecutableMemberDoc member) {
	...
	List<RPL> readRPLs = member.readRPLs();
	List<RPL> writeRPLs = member.writeRPLs();
	if (readRPLs.nonEmpty()) {
	    writer.println();
	    writer.print(indent);
	    writer.print("reads  ");
	    String readIndent = indent + "       ";
	    writer.print(readRPLs.head);
	    for (RPL rpl : readRPLs.tail) {
		writer.println(",");
		writer.print(readIndent);
		writer.print(rpl);
	    }
	}
	if (writeRPLs.nonEmpty()) {
	    writer.println();
	    writer.print(indent);
	    writer.print("writes ");
	    String writeIndent = indent + "       ";
	    writer.print(writeRPLs.head);
	    for (RPL rpl : writeRPLs.tail) {
		writer.println(",");
		writer.print(writeIndent);
		writer.print(rpl);
	    }
	}
    }
