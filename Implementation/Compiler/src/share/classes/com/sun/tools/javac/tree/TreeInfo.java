...
public class TreeInfo {
	...
	case(JCTree.TYPEAPPLY):
	    return getStartPos(((JCTypeApply) tree).functor);
	...
	case JCTree.TYPEAPPLY:
	    return name(((JCTypeApply) tree).functor);
	default:
	    return null;
	}
    }
	...
	case JCTree.TYPEAPPLY:
	    return symbol(((JCTypeApply) tree).functor);
	default:
	    return null;
	}
    }
	...
	int flag = 1;
        while ((flag & DPJStandardFlags) != 0 && (flag & flags) == 0)
	    flag = flag << 1;
        return flag;
    }
	...
	return Flags.toString(flags & DPJStandardFlags).trim();
    }
