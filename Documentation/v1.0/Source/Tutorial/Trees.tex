...
\begin{itemize}
%
\item Method \kwd{partition} (lines 21--27) is a helper method that
...
%
\item Method \kwd{makeTree} (lines 28 and following) does the actual
...
\begin{itemize}
%
\item If the input array is empty, return \kwd{null} (line 31).
%
...
\item In parallel, populate the left and right children of the new
  inner node, by calling \kwd{makeTree} recursively (lines 37--42).
%
...
Notice that line 38 calls \kwd{makeTree} is called with
$\kwd{RN}=\kwd{RN:InnerNode}$, which creates the right type for
assigning into \kwd{node.leftChild}.  Notice also that the effects of
the two branches of the \kwd{cobegin} are \kwd{reads RA:* writes
...
instead of the regions \kwd{LeftRgn} and \kwd{RightRgn} to distinguish
...
(\kwd{Benchmarks/Applications/Barnes-Hut/dpj}) builds an octtree for
...
\kwd{updateCenterOfMass} on the left and right nodes (lines 13--18),
and constructs a new \kwd{Body} representing the center of mass (lines
19--26).  It then stores the \kwd{Body} representing the center of
mass into the input node and returns the \kwd{Body}.

In lines 14--17, the effects of the two \kwd{cobegin} statements are
