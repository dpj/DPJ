...
\kwd{cobegin} statement, and the join point is at the end of the
statement.  Execution does not continue after the \kwd{cobegin}
...
which the \kwd{cobegin} statement in lines 10--13 were replaced with
...
effect \kwd{writes Y} from the summary of \kwd{setXAndY}, because of
the write to region \kwd{Y} in line 12.
...
\item Any read or write of a local variable (i.e., a variable declared
  ...
  instance, the read of parameter \kwd{x} in line 10 doesn't generate
...
\item Any effect on a \kwd{final} variable.  That's because the value
  of a \kwd{final} variable never changes after it is initialized, so
...
Try compiling the \kwd{Point} class shown in
...
lines 11 and 12 in the \kwd{cobegin}.
...
\kwd{A:C} (lines 3--4).  In line 6, we use the partially specified RPL
...
(\kwd{A:B} is nested under \kwd{A}) and \emph{inclusion} (\kwd{A:B} is
included in \kwd{A:*} but not in \kwd{A}).  It may help to think of
RPLs like a file system.  For example, given the UNIX path
\kwd{foo/bar}, the file system is a tree with \kwd{bar} nested under
\kwd{foo}; but \kwd{rm foo} won't remove \kwd{bar}; you have to say
either \kwd{rm foo/bar} or \kwd{rm foo/*}.
...
field \kwd{x} is located in \kwd{R}.  That means that when an object
of type \kwd{Data<r>} is created, for some actual region \kwd{r}, its
\kwd{x} field will reside region \kwd{r}.
...
\kwd{updateBoth}.  In line 19, the effect is \kwd{writes First}.
...
\item As a last resort, use a type cast (Reference Manual
  \S~5.1.3). For example, \kwd{b = (C<B>) a}.  This is usually not a
...
\cutname{arrays.array-classes.html}

DPJ v1.1 introduces a feature called an \emph{array class} to support
...
length, e.g., \kwd{new ArrayInt<r>(10)}.  They are accessed using the
...
array class defines an array of \kwd{int} such that cell \kwd{i} is in
region \kwd{[i]}:
%
\begin{dpjlisting}
...
runtime value of $e$.  Every array class has an \kwd{index} field that
refers to the index of a cell.

The two strategies can also be mixed.  For example, you can give array
\kwd{A} the type \kwd{IPArrayInt<rA>} and array \kwd{B} type
\kwd{IPArrayInt<rB>}.  Then if you want to write in parallel to the
...
\subsubsection{DPJ Runtime Classes
\label{sec:arrays:runtime}}
\cutname{arrays.runtime.html}
...
\kwd{ArrayInt} and \kwd{IPArrayInt} shown above for each of the Java
primitive types; there are also generic versions.  Second, the DPJ
runtime provides classes \kwd{ArraySlice} and \kwd{Partition} for
