...
\bfhead{Arrays of primitive and class types} There are two cases to
consider:
%
\begin{enumerate}
%
...
\bfhead{Arrays of arrays} An array of arrays is an array of objects
...
%
\begin{description}
\item \emph{selected-exp}\kwd{.}\emph{field-name}
\end{description}
%
...
an expression \emph{selected-exp}\kwd{.}\emph{field-name}:
%
\begin{enumerate}
%
...
%
\item If \emph{selected-exp} is a \kwd{final} local variable or
...
\kwd{return} statement in line 5.  Step 1: The selected expression is
a \kwd{new} expression of type \kwd{FieldTypingExample<r>}.  Step 2:
The field name is \kwd{field1}, the class $C$ is
\kwd{FieldTypingExample}, and the declared type of the field is
\kwd{FieldTypingExample<R>} (line 3).  Step 3: Nothing to do (see
\S~\ref{sec:types:exp:capture}).  Step 4: Substituting \kwd{r} given
in the selected type expression for \kwd{R} in the field type, we have
that the answer is \kwd{FieldTypingExample<r>}.

Now let's see how to compute the type of the expression in the
\kwd{return} statement in line 10.  Step 1: The selected expression is
\kwd{arg}, which has type \kwd{FieldTypingExample<R>}.  Step 2: Now
the field is \kwd{field2}, and its type is
\kwd{FieldTypingExample<this>} (line 7).  Step 3: Again, nothing to
do.  Step 4: \kwd{arg} is a \kwd{final} local variable (line 9), so we
can substitute it for \kwd{this} in the type of \kwd{field2}.
Therefore the answer is \kwd{FieldTypingExample<arg>}.
...
%
\begin{description}
\item \emph{selected-exp}\kwd{.<}\emph{type-args}\kwd{,}\emph{rpl-args}\kwd{>}\emph{method-name}\kwd{(}\emph{args}\kwd{)}
\end{description}
%
...
In this example, the captured type of the selected expression is
