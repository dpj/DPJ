...
%
\begin{description}
\item \kwd{dpj Foo.java \$\{DPJ\_ROOT\}/Implementation/Runtime/dpj/*java}
\end{description}
%
...
\bfhead{Generic \kwd{ArraySlice}} The generic \kwd{ArraySlice} class
...
\ithead{Creating an \kwd{ArraySlice}} There are three ways to create a
...
array and you want to make an \kwd{ArraySlice} out of it.  So the second
way to create an \kwd{ArraySlice} is to call a constructor that takes an
...
The third way to create an \kwd{ArraySlice} is to make a \emph{subslice}
(i.e., a slice) of an existing \kwd{ArraySlice}.  This is explained
below.
...
array.  To create a subslice of an \kwd{ArraySlice}, you call the
\kwd{subslice} instance method with a start and length argument.  For
example, the following code creates an \kwd{ArraySlice} and then extracts
...
\bfhead{Specialized \kwd{ArraySlice}s} Because Java does not support
binding primitive types to generic parameters, the DPJ runtime also
has versions of \kwd{ArraySlice} specialized to the primitive types
(\kwd{DPJInt} for \kwd{int}, \kwd{DPJBoolean} for \kwd{boolean},
etc.).  These operate identically to the generic \kwd{ArraySlice},
except that the element type is given by the class, and there is no
generic parameter.

Another way to write such arrays is to create a generic \kwd{ArraySlice}
using the class corresponding to the primitive type.  For example,
instead of an \kwd{ArraySliceInt}, one could use a
\kwd{ArraySlice<Integer>}, as described above.  This works, but the code
...
the partition.  As with \kwd{ArraySlice}, there is a generic version,
and there are specialized versions.

\bfhead{Generic \kwd{Partition}}  The generic \kwd{Partition}
class represents an array of subslices of an \kwd{ArraySlice}.  It has
...
\ithead{Creating a \kwd{Partition}} There are several ways to
...
\bfhead{Specialized \kwd{Partition}s} As with \kwd{ArraySlice},
there are versions of \kwd{Partition} specialized to the various
